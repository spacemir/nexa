...
#include "NetworkTemporal3BCPNN_IF.h"
#include "NetworkTriesch.h"
#include "NetworkFoldiak.h"

//using namespace std;

void NetworkTemporal3BCPNN_IF::NetworkSetupStructure()
{
	ofstream wout;
	...
	m_layer4 = new PopulationColumns(this,m_nrHypercolumns3,m_nrRateUnits3,PopulationColumns::IF,MPIDistribution::ParallelizationHypercolumns); 
	OneToOneConnectivity* one10 = new OneToOneConnectivity(false);
	m_layer4->AddPre(m_layer3,one10); // Feedforward
	m_lin=new ProjectionModifierHebbSimple();
	one10->AddProjectionsEvent(m_lin);
	m_lin->SwitchOnOff(false);
	AddPopulation(m_layer3);
	AddPopulation(m_layer4);
...
void NetworkTemporal3BCPNN_IF::NetworkSetupMeters()
{
	...
	m_activityMeter2 = new Meter("IFOutput.csv",Storage::CSV);
	m_activityMeter2->AttachPopulation(m_layer4);
	//m_activityMeter->SwitchOnOff(false);
	AddMeter(m_activityMeter);
	AddMeter(m_activityMeter2);
...
void NetworkTemporal3BCPNN_IF::NetworkSetupParameters()
{
	// not used
}


vector<float> NetworkTemporal3BCPNN_IF::toBinary(const vector<float>& data, int nrHc, int nrMc)
{
...
void NetworkTemporal3BCPNN_IF::ComputeCorrelation(const vector<vector<float> >& trainingData,  PopulationColumns* inputLayer, StructureMIMDSVQ* structure, int iter) {
...
bool NetworkTemporal3BCPNN_IF::ComputeMDS(const vector<vector<float> >& trainingData,  PopulationColumns* inputLayer, StructureMIMDSVQ* structure, int iter) {
...
void NetworkTemporal3BCPNN_IF::ComputeVQ(const vector<vector<float> >& trainingData,  PopulationColumns* inputLayer, StructureMIMDSVQ* structure, int iter) {
...
bool NetworkTemporal3BCPNN_IF::ExtractFeatures(const vector<vector<float> >& trainingData,  PopulationColumns* inputLayer, StructureMIMDSVQ* structure) {
...
void NetworkTemporal3BCPNN_IF::TrainBCPNN(const vector<vector<float> >& trainingData, const vector<vector<float> >& trainingLabels, PopulationColumns* inputLayer, int iterationsBCPNN) {
	int prevLabel=-1;
...
void NetworkTemporal3BCPNN_IF::TrainLayer(const vector<vector<float> >& trainingData, PopulationColumns* inputLayer, StructureMIMDSVQ* structure, int iterationsCorrs, int iterationsMDS, int iterationsVQ, int iterationsFeatures)
{
	// Training phase
...
void NetworkTemporal3BCPNN_IF::NetworkRun()
{
	int nrColors = 2;

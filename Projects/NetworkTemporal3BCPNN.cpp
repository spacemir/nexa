...
void NetworkTemporal3BCPNN::NetworkSetupStructure()
{
	...
	m_architecture = PC;
	m_nrInputHypercolumns = 570;
	m_nrInputRateUnits = 1;
	m_verbose=true;
	// layer 2
	m_nrHypercolumns2 = 40;//24;//96;
	m_nrRateUnits2 = 40;//100;
...
void NetworkTemporal3BCPNN::ComputeCorrelation(const vector<vector<float> >& trainingData,  PopulationColumns* inputLayer, StructureMIMDSVQ* structure, int iters) {
...
void NetworkTemporal3BCPNN::ComputeMDS(const vector<vector<float> >& trainingData,  PopulationColumns* inputLayer, StructureMIMDSVQ* structure, int iters) {
...
void NetworkTemporal3BCPNN::ComputeVQ(const vector<vector<float> >& trainingData,  PopulationColumns* inputLayer, StructureMIMDSVQ* structure, int iters) {
...
void NetworkTemporal3BCPNN::ExtractFeatures(const vector<vector<float> >& trainingData,  PopulationColumns* inputLayer, StructureMIMDSVQ* structure, int iters) {
...
void NetworkTemporal3BCPNN::NetworkRun()
{
	...
	if(m_architecture == PC)
	{
		...
		filenameTrain="D:\\Databases\\emails\\tfidf_train2.csv";
	...
	int iterationsCorrs = 10*trainingData.size(); // run through x times, 5

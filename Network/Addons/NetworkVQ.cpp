...
void CSL::NewCodeVector(vector<float>& oldCodeVector, vector<float>& inputVector)
{
	//float eta = 0.01;
	for(int i=0;i<oldCodeVector.size();i++) {
		oldCodeVector[i] = oldCodeVector[i] + m_eta*(inputVector[i] - 
			oldCodeVector[i]);
	}
}

// application specific: sensor drift
void CSL::NewCodeVectorVector(vector<float>& oldCodeVector, vector<float>& inputVector, vector<float>& eta)
{

	//float eta = 0.01;
	for(int i=0;i<oldCodeVector.size();i++) {
		oldCodeVector[i] = oldCodeVector[i] + eta[i]*(inputVector[i] - 
			oldCodeVector[i]);
	}
}
...
{
	int index = GetClosestCodeVectorIndex( x, m_c);
	NewCodeVectorVector( m_c[ index ], x, eta );
}
...
void CSL::ParallelDistortionCalculation(float *distortion, vector<float>& D)
{
	...
	else
	{
		*distortion = newDistortion;
		totD = subD;
		//MPI_Allreduce(&newDistortion, distortion, 1, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
		//MPI_Allreduce(&subD[0], &totD[0], m_c.size(), MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
	}
...
//		m_winners = new int[n_s];
	...
	for (int i = 0; i < m_x->size();i++)//m_maxUsedNodes*n_s; i++ )
	{
		...
		NewCodeVector( m_c[ m_total_winners[order[i]] ], (*m_x)[ order[i] ] );
	}      	
}
...
int ProjectionModifierCSL::m_number = 0;
...
void ProjectionModifierCSL::RecordCodeVectors() {
	...
	if(mpirank==0) {
		for(int i=0;i<m_csl.size();++i) {
			vector<vector<float> > ccv = *(m_csl[i]->GetCodeVectors());
		...
		for(int i=1;i<mpisize;++i) {
			int ncsl;
			MPI_Recv(&ncsl,1,MPI_INT,i,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
			for(int k=0;k<ncsl;++k) {
				int data[2];
				MPI_Recv(data,2,MPI_INT,i,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
				for(int j=0;j<data[0];++j) {
					vector<float> d(data[1],0.0f);
					MPI_Recv(&d[0],data[1],MPI_FLOAT,i,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
					cv.push_back(d);
				}
			}
		}
		char* fname = new char[30];
		sprintf(fname,"cv_%d.csv",m_id);
		Storage* store = new Storage();
		store->SaveDataFloatCSV(fname,cv,Storage::Standard, "");
	}
	else {
		int ncsl = m_csl.size();
		MPI_Send(&ncsl,1,MPI_INT,0,0,MPI_COMM_WORLD);
		for(int i=0;i<m_csl.size();++i) {
			cv=*(m_csl[i]->GetCodeVectors());
			int data[2];
			data[0]=cv.size();
			if(cv.size()>0)
				data[1]=cv[0].size();
				...
				data[1]=0;
			MPI_Send(&data[0],2,MPI_INT,0,0,MPI_COMM_WORLD);
			for(int j=0;j<data[0];++j) {
				vector<float> d = cv[j];
				MPI_Send(&d[0],data[1],MPI_FLOAT,0,0,MPI_COMM_WORLD);
			}
		}
	}
}